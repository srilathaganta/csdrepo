/*! tableExport.js
 *  using this, downloads happen at client side
 *  2017-10-8
 *
 *  By Ganta Srilatha
 *  
...
/*! @source https://github.com/srilathaganta/csdrepo/blob/master/DataGridWithAngularJs/WebContent/Scripts/tableExport.js */

/*!
 * utility function to call a dynamic function using callback.
 * 
 * Constructs an utility callback function 'utilityCallback' with 
 * empty object and specified callback function name.
 *  
 * return a call to client defined dynamic function 
 * 
 */
function utilityCallback(object , callbackfuncName) {
	return callbackfuncName (object);
}

/*!
 * function to check if the passing object is null or not.
...
/*!
 * default properties defined and using in UI while loading the page.
 * 
 */
var defaults = {
    separator: ',',
...
PdfdefaultsInf.customText=utilityCallback (object, addText);
...
function tableAllExport(type,dsDetail, jsonDataVal,tobj, imageCode) {
	debugger
       ...
       if (dsDetail == 'server') {
        ...
        if (dsDetail == 'server') {
            ...
            //code added for Dynamic data generate in pdf using callback - starts
            if(checkObject(PdfdefaultsInf.customText)){
            	var count =0;
            ...
            //code added for Dynamic data generate in pdf using callback - ends
           
            if (tablecount == 0) { tablestartpos = parseInt(PdfdefaultsInf.customtextLines) * 10+count } else { tablestartpos = parseInt(doc.autoTable.previous.finalY) + 10 }
        ...
        doc.addPage();
		doc.addImage(imageCode, 'JPEG', 0, 0);
